...
# + return - The content type to set on content-delivery requests for this topic
public isolated function getTopicContentType(string topicName) returns string {
    lock {
        common:TopicRegistration? topicRegistration = registeredTopicsCache[topicName];
        if topicRegistration is () {
            return common:DEFAULT_CONTENT_TYPE;
        }
        return topicRegistration?.contentType ?: common:DEFAULT_CONTENT_TYPE;
    }
}
